...
APIGET="/bin/curl -s GET -k -u ${USER}:${PASS}"
...
RHOST=`echo $HN_FQDN | tr '[:upper:]' '[:lower:]' | tr -d '[;!@#$%^&*()+=|{}[] ]'`
...
#echo "hammer -u $USER -p $PASS host info --name $1 | grep ^Id: | awk '{print $NF}'"
...
if [[ -n $HOSTID ]] && [[ -n $HGID ]]; then
   ...
   #echo hammer -u $USER -p $PASS host update --hostgroup-id $HGID --id $HOSTID --puppet-ca-proxy $RHSS --comment $COMMENT
   #curl -s -H "Accept:application/json,version=2" -H "Content-Type:application/json" -X PUT -k -u ${USER}:${PASS} -d "{\"host\":{\"comment\":\"${COMMENT}\", \"hostgroup_id\":$HGID}}" https://${SATELLITE}/api/hosts/${HOSTID}
