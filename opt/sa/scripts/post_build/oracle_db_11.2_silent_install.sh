...
RSPDIR=/opt/sa/scripts/post_build/oracle_rsp
...
ASMD_REQ="DATA01 FLASH01"

# Restart ASM to make sure it's running
...
if [[ ! -x "$ASM_INSTALLER" ]]; then
   echo "Error: $ASM_INSTALLER" not present or not executable | tee -a $LOGFILE
   ls -ald $ASM_INSTALLER
fi
