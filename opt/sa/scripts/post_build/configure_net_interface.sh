...
sed -i "/^DHCP_HOSTNAME=.*$/d" ${CONFDIR}/ifcfg-${UPINT}
sed -i "/^DNS/d" ${CONFDIR}/ifcfg-${UPINT}
#sed -i "s/^HWADDR=.*$//" ${CONFDIR}/ifcfg-${UPINT}
sed -i "/^HWADDR=.*$/d" ${CONFDIR}/ifcfg-${UPINT}
...
# Add ethtool option to disable large-receive-offload

if [[ -n `grep ^ETHTOOL_OPTS ${CONFDIR}/ifcfg-${UPINT}` ]]; then
   sed -i '/^ETHTOOL_OPTS=/d' ${CONFDIR}/ifcfg-${UPINT}
fi
echo "ETHTOOL_OPTS=\"-K \${DEVICE} lro off\"" >> ${CONFDIR}/ifcfg-${UPINT}
